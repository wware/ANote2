...
//clase usada para la búsqueda y descarga del pdf de un artículo dados sus datos
//(pmid, enlaces de partida en la búsqueda y un trozo de texto del artículo usado para validación)
public class WebConnectionConsola {
	private static final String DEFAULT_DOWNLOAD_DIR = "./Docs/";
	private static int profundidad_max = 2; // profundidad máxima de enlaces
											// HTML a mirar
	// partiendo de la profundidad 0
	private static Vector<URL> urls_antiguas = new Vector<URL>();	// url's descargadas
	private static DatosArticulo art;	// datos del artículo necesarios en la
										// búsqueda del pdf
	private static String ruta_id;		// ruta de la carpeta donde se almacenará el
										// pdf del artículo
	...
	static {
		...
		// todas las cookies en una única petición
	...
	// artículo dados (indicando finalmente si se ha encontrado o no el pdf)
	...
	// datos del artículo dados (indicando finalmente si se ha encontrado o no el pdf)
	public static synchronized boolean buscarPDF(String ruta_descargas, DatosArticulo artic, PDFDownloadHandler handler) {
		...
		// si no se dispone del abstract o el título del artículo se termina
		// porque no se puede comprobar que lo que se descargue sea el texto
		// completo
		if (!art.tieneTrozoTexto()) {
			handler.error("No se dispone del abstract ni del título del artículo, por tanto no se busca su texto completo");
//			System.out.println("No se dispone del abstract ni del título del artículo, por tanto no se busca su texto completo");
...
//		System.out.println(art.getTrozo_texto().getTexto());
		...
		try {
			// crea la carpeta en la que se almacenarán las descargas
			Comun.crear_carpeta(dir);

			String url;
			// para cada url de partida se busca el pdf del artículo
			for (int i = 0; i < urls.length; i++) {
				try {
					...
					// pdf
					// deben tener una profundidad máxima superior en 1 a la
					// habitual
...
//			e.printStackTrace();
		...
		// descargaría
		//Comun.borrarFicheroRecursivo(dir);
		handler.finished(null);
		return false;
	}

	// dada la url inicial realiza la búsqueda y descarga de ficheros para
	// obtener el pdf
	// (devuelve true o false según haya encontrado o no el pdf del artículo)
	private static boolean algoritmo(URL url_inicial, PDFDownloadHandler handler) throws Exception {
		...
		int nuevo_nivel = 1; // nº de urls que faltan por descargar en la
								// profundidad actual
		handler.info("profundidad " + profundidad);
//		System.out.println("profundidad " + profundidad);
		AnalisisPaginaWeb.getUrls_a_mirar().add(url_inicial);

		// Se descarga y analiza el código de las url's a descargar mientras
		// haya url's que
		// descargar y no se haya alcanzado la profundidad máxima y no se haya
		// encontrado el pdf
...
//				System.out.println("profundidad " + profundidad);
			...
			nuevo_nivel--;
			if (url != null) {
				// si ya se había analizado la página de la url no se vuelve a
				// descargar
				...
				// si la url final de la página descargada es distinta de la
				// inicial
				// se añade a urls_antiguas si no estaba y si ya estaba no se
				// analizará
				if (nueva_url != null && !url.sameFile(nueva_url)) {
					// se pone a null, para no entrar en el bloque de análisis,
					// pero sí borrar el fichero de la página descargada
				...
				if (file.exists()) {
					if (nueva_url != null) {
						...
						// si el fichero descargado es un pdf (no tiene porqué
						// ser el texto completo)
						if (Comun.esPDF(cad)) {
							// no se avanzará más en profundidad porque se
							// supone que el pdf del artículo
							// debería estar al mismo nivel o menor que el de
							// cualquier otro pdf
							profundidad_max = profundidad;
							// si el trozo de texto disponible está contenido en
							// el pdf descargado
						...
						// si el fichero descargado es HTML se analizará la
						// página en busca de
	...
	// descarga el código fuente de la url indicada en el fichero indicado
...
//		System.out.println("Bajando " + url_cad + " a " + fich);
		...
		try {
			...
			try {
				...
				// si se obtiene algún código de error se devuelve null
...
//					System.err.println("Method failed: " + method.getStatusLine());
				...
				// se va descargando la página y guardándola en el fichero
				// indicado
...
//			e.printStackTrace(System.err);
	...
	// comprueba si el trozo de texto disponible está contenido en el texto del
	// fichero pdf
	...
	private static boolean tieneTrozoTexto(String archivo, PDFDownloadHandler handler) {
		...
		// si se dispone del abstract se buscará dicho abstract en los 10000
		// primeros
		...
		// si se dispone del título se buscará dicho título en los 200 primeros
		// caracteres del
		// texto (o si el título es muy largo en un espacio de 4 veces la
		// longitud del título)
		else {
