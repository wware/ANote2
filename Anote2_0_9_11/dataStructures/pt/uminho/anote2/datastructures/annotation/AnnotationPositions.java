			...
			if(p.equals(position))
			{
				...
				if(annotations.get(p) instanceof IEntityAnnotation)
				{
					...
					else
					{
						...
						if(position.getOriginalText()!=null && position.getOriginalText().length()>0)
						{
							...
							if(original.equals(ent.getAnnotationValue()))
							{
								return false;
							}
							else
							{
								...
								ent = (IEntityAnnotation) entityAnnotation;
								if(original.equals(ent.getAnnotationValue()))
								{
									annotations.put(p, ent);
									return true;
								}
								else
								{
