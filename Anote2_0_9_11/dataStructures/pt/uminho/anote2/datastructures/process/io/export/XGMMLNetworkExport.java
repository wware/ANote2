...
import pt.uminho.anote2.datastructures.report.processes.io.export.NetworkExportReport;
import pt.uminho.anote2.datastructures.textprocessing.XmlConversions;
import pt.uminho.anote2.process.IE.io.export.INetworkExport;
...
public class XGMMLNetworkExport implements INetworkExport {
	...
	public INetworkExportReport export() {
		...
		try {
			...
			e.printStackTrace();
			report = new NetworkExportReport("Network Export", 0, 0);
			report.setcancel();
			return report;
		}
		report = new NetworkExportReport("Network Export", network.getNodes().size(), network.getEdges().size());
		return report;
	}
	...
	private void writeEdge(BufferedWriter bw, IEdge edge) throws IOException {
		...
		{
			IAtributes attInteraction = new Atributes("interaction", "interaction", XmlConversions.removeSpecialCharacteres(edge.getLabel()), "string", false, null);
			writeAtribulte(bw, prefix, attInteraction );
		}
		if(graphicsAtribute!=null)
		{
			edge.getAtributesList().add(new Atributes(null, "Weight", String.valueOf(graphicsAtribute.getWith()), "integer", false, null));		
		}
	...
	private void writeNode(BufferedWriter bw, INode node) throws IOException {
		...
		if(graphicsAtribute!=null)
		{
			node.getAtributesList().add(new Atributes(null, "Weight", String.valueOf(graphicsAtribute.getWith()), "integer", false, null));		
		}
