...
public class WorkflowQueryOperation {
	...
	public void getOutput(QueryInformationRetrievalExtension query)
	{

	}
	
	@Port(name="options",direction=Direction.INPUT,order=2)
	public void getOutput(List<Object> list)
	{
		System.gc();
		report = new GeneralReportGUI("Workflow Information Extraction (From Query) - Report");
		createdBy = "Workflow Information Extraction (From Query) on date "+Utils.currentTime();
		list.remove(1);
		try {
			processinformation(list);
			report.viewReport();
		} catch (InternetConnectionProblemException e) {
			new ShowMessagePopup("Workflow Information Extraction (From Query) Fail");
	...
	private void processinformation(List<Object> list) throws SQLException, DatabaseLoadDriverException, Exception {
		...
		if(totalSteps > 1 && !cancel)
		{
			...
			createCorpus(list.get(0),list.get(1));
		}
	...
	private void createCorpus(Object pubs, Object confuguration) throws SQLException, DatabaseLoadDriverException {
		if(confuguration instanceof ICorpusCreateConfiguration)
		{
			ICorpusCreateConfiguration conf = (ICorpusCreateConfiguration) confuguration;
			Set<IPublication> puns = (Set<IPublication>) pubs;
			List<IPublication> pubsList = new ArrayList<IPublication>(puns);
			IIRCrawlingProcessReport rep = WorkflowCreateCorpus.processIRCrawl(crawl,conf, pubsList,progress);
			if(rep!=null)
			{
			...
			ICorpusCreateReport reportCreateCorpus = WorkflowCreateCorpus.processCreateCorpus(project,conf,pubsList,progress);
