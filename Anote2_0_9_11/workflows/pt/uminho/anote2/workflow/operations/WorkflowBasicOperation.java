...
@Operation
public class WorkflowBasicOperation {
	...
	{
		System.gc();
		report = new GeneralReportGUI("Workflow Information Retrieval and Extraction (Basics) - Report");
		createdBy = "Workflow Information Retrieval and Extraction (Basics) on date "+Utils.currentTime();
		try {
			processinformation(list);
			report.viewReport();
		} catch (InternetConnectionProblemException e) {
			new ShowMessagePopup("Workflow Information Retrieval and Extraction (Basics) Fail");
	...
	private void processinformation(List<Object> list) throws InternetConnectionProblemException, DatabaseLoadDriverException, SQLException ,Exception{
	...
	private List<String> calculateSteps(List<Object> list) {
		...
		if(list.size() > 1)
		{
			...
			{
				ICorpusCreateConfiguration conf = (ICorpusCreateConfiguration) object;
				if(!conf.getCorpusTextType().equals(CorpusTextType.Abstract) && conf.processJournalRetrievalBefore())
				{
