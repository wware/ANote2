...
public class WorkflowGeneralRESettings extends JPanel{
	...
	public WorkflowGeneralRESettings(Map<String, Object> initial_props) {
		super();
	...
	private void defaultSettings() {
		jComboBoxAvailableRE.setSelectedItem(initial_props.get(WorkflowGeneralDefaulSettings.RE));
		changePanel();
	}
	...
	private void initGUI() {
		{
			...
			{
				jPanelSettingsPanel = (JPanel) REProcessChangeSettingBox.getInstance().getChangeSettings(WorkflowsEnum.GENERAL, REWorkflowProcessesAvailableEnum.Relation);
	...
	public Map<String,Object> getProperties() {
		...
		result.put(WorkflowGeneralDefaulSettings.RE, jComboBoxAvailableRE.getSelectedItem());
		for(int i=0;i<jComboBoxAvailableRE.getItemCount();i++)
		{
			result.putAll(REProcessChangeSettingBox.getInstance().getChangeSettings(WorkflowsEnum.GENERAL, REWorkflowProcessesAvailableEnum.Relation).getProperties());
		}
		return result;
	}

	public boolean haveChanged() {
		if(initial_props.get(WorkflowGeneralDefaulSettings.RE).equals(jComboBoxAvailableRE.getSelectedItem()))
		{
			return true;
		}
		for(int i=0;i<jComboBoxAvailableRE.getItemCount();i++)
		{
			if(REProcessChangeSettingBox.getInstance().getChangeSettings(WorkflowsEnum.GENERAL, REWorkflowProcessesAvailableEnum.Relation).haveChanged())
			{
				return true;
			}
		}
