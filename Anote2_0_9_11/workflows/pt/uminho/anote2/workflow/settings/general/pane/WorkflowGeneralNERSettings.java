...
public class WorkflowGeneralNERSettings extends JPanel{
	...
	public WorkflowGeneralNERSettings(Map<String, Object> initial_props) {
		super();
	...
	private void defaultSettings() {
		jComboBoxAvailableNER.setSelectedItem(initial_props.get(WorkflowGeneralDefaulSettings.NER));
		changePanel();
	}
	...
	private void initGUI() {
		{
			...
			{
				jPanelSettingsPanel = (JPanel) NERProcessChangeSettingBox.getInstance().getChangeSettingsAndUpdate(WorkflowsEnum.GENERAL, (NERWorkflowProcessesAvailableEnum)jComboBoxAvailableNER.getSelectedItem());
	...
	public Map<String,Object> getProperties() {
		...
		result.put(WorkflowGeneralDefaulSettings.NER, jComboBoxAvailableNER.getSelectedItem());
		for(int i=0;i<jComboBoxAvailableNER.getItemCount();i++)
		{	
			result.putAll(NERProcessChangeSettingBox.getInstance().getProperties(WorkflowsEnum.GENERAL, (NERWorkflowProcessesAvailableEnum)jComboBoxAvailableNER.getSelectedItem()).getProperties());
		}
		return result;
	}

	public boolean haveChanged() {
		if(!initial_props.get(WorkflowGeneralDefaulSettings.NER).equals(jComboBoxAvailableNER.getSelectedItem()))
		{
			return true;
		}
		for(int i=0;i<jComboBoxAvailableNER.getItemCount();i++)
		{
			if(NERProcessChangeSettingBox.getInstance().getProperties(WorkflowsEnum.GENERAL, (NERWorkflowProcessesAvailableEnum)jComboBoxAvailableNER.getSelectedItem()).haveChanged())
			{
				return true;
			}
		}
