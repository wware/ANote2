...
public class WorkflowQueryNERSettings extends JPanel{
	...
	public WorkflowQueryNERSettings(Map<String, Object> initial_props) {
		super();
	...
	private void defaultSettings() {
		jComboBoxAvailableNER.setSelectedItem(initial_props.get(WorkflowQueryDefaulSettings.NER));
		changePanel();
	}
	...
	private void initGUI() {
		{
			...
			{
				jPanelSettingsPanel = (JPanel) NERProcessChangeSettingBox.getInstance().getChangeSettingsAndUpdate(WorkflowsEnum.QUERY, (NERWorkflowProcessesAvailableEnum)jComboBoxAvailableNER.getSelectedItem());
	...
	public Map<String,Object> getProperties() {
		...
		result.put(WorkflowQueryDefaulSettings.NER, jComboBoxAvailableNER.getSelectedItem());
		for(int i=0;i<jComboBoxAvailableNER.getItemCount();i++)
		{
			result.putAll(NERProcessChangeSettingBox.getInstance().getProperties(WorkflowsEnum.QUERY, (NERWorkflowProcessesAvailableEnum)jComboBoxAvailableNER.getSelectedItem()).getProperties());
		}
		return result;
	}

	public boolean haveChanged() {
		if(!initial_props.get(WorkflowQueryDefaulSettings.NER).equals(jComboBoxAvailableNER.getSelectedItem()))
		{
			return true;
		}
		for(int i=0;i<jComboBoxAvailableNER.getItemCount();i++)
		{
			if(NERProcessChangeSettingBox.getInstance().getProperties(WorkflowsEnum.QUERY, (NERWorkflowProcessesAvailableEnum)jComboBoxAvailableNER.getSelectedItem()).haveChanged())
			{
				return true;
			}
		}
