...
public class NERExportCSVGUI extends DialogGenericViewInputGUI{
	...
	private NERSchema process;
	
	public NERExportCSVGUI()
	{
		process = (NERSchema) HelpAibench.getSelectedItem(NERSchema.class);
		initGUI();
		this.setTitle("Export NER Schema to TSV File");
		this.setModal(true);
	}
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					else
					{
						...
						if(save_path.isDirectory())
						{
							...
							jTextFieldTextChooser.setText(save_path.getAbsolutePath()+"/"+"ner_"+process.getID()+".tsv");
						}
	...
	public void init(ParamsReceiver arg0, OperationDefinition<?> arg1) {
		Object obj = HelpAibench.getSelectedItem(NERSchema.class);
		if(obj==null)
		{
			Workbench.getInstance().warn("No NERProcess selected on clipboard");
			dispose();
		} else
			try {
				if(((NERSchema) obj).getAllEntities().size() == 0)
				{
	...
	protected void okButtonAction() {
		...
		else 
		{
			this.paramsRec.paramsIntroduced( new ParamSpec[]{
					new ParamSpec("process", NERSchema.class,process, null),
	...
	protected String getHelpLink() {
		return GlobalOptions.wikiGeneralLink +"NERProcess_Export_To_CSV_File";
	}
