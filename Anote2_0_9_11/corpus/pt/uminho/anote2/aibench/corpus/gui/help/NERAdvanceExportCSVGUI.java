...
public class NERAdvanceExportCSVGUI extends DialogGenericView{
	...
	private INERSchema process;
	
	public NERAdvanceExportCSVGUI(INERSchema process)
	{
		this.process = process;
		initGUI();
		this.setTitle("Export NER Schema to CSV File");
	...
	protected void okButtonAction() {
		...
		else 
		{
			ParamSpec[] paramsSpec = new ParamSpec[]{
					new ParamSpec("process", INERSchema.class,process, null),
					new ParamSpec("file", File.class,new File(jTextFieldTextChooser.getText()), null)
			};
			for (@SuppressWarnings("rawtypes") OperationDefinition def : Core.getInstance().getOperations()){
				if (def.getID().equals("operations.exportnetocsv")){			
